...
The <strong>set</strong> of all considered inputs to the function or variable(s) for which there is a val;id evaluation according to the rules is called the <strong>domain</strong> of the function or variable(s)
...
### Set-Roster Notation
A set may be specified using <strong>set-roster notation by writing all of its elements between braces. For example, the set $A = {1, 2, 3}$ denotes the set whose elements are 1, 3, and 3

A variation of the notation is sometimes used to describe a very large set, as the set 
$A = {1, 2, 3, ..., 100}$ regers to the set of all integers from 1 to 100. The symbol ... is called an <strong>ellipsis</strong> and is read "and so forth"
...
a convenient way to refer to sets is using capital letters
